...
def change_cart_handlers(call):
    ...
    cart = TelegramProductCartCounter.objects.filter(
        Q(user__chat_id=call.message.chat.id) & Q(counter=False))
...
def del_product_in_cart_handlers(call):
    ...
    cart_product = TelegramProductCartCounter.objects.get(
        Q(user__chat_id=call.message.chat.id) & Q(counter=False) & Q(product__slug=product_slug))
    if call.data.split('~')[-1] == 'yes':
        ...
        return change_cart_handlers(call)
    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                          text=f'Вы уверены что хотите удалить из корзины {cart_product}?', reply_markup=yes_no_keyboard(call.data))
    # bot.change_message
...
def del_all_product_in_cart_handlers(call):
    ...
    if call.data.split('~')[-1] == 'yes':
        ...
        bot.send_message(call.message.chat.id,
                         'Корзина успешно очищена', reply_markup=main_keyboard())
        return
    elif call.data.split('~')[-1] == 'no':
        return change_cart_handlers(call)
    # bot.delete_message(call.message.chat.id, call.message.message_id)
